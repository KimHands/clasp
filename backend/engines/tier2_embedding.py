...
# 형식(문서 유형) + 내용(과목·주제) 모두 포함하여 구체적인 태그 부여
TAG_CANDIDATES = {
    "문서": [
        # 문서 형식
        ...
        "매뉴얼", "제안서", "설명서", "협약서", "공문",
        # 학업 관련
        "과제", "레포트", "실험보고서", "실습보고서", "팀프로젝트",
        "기말과제", "중간과제", "졸업논문", "학술논문", "연구보고서",
        # 과목·전공 분야
        "운영체제", "자료구조", "알고리즘", "컴퓨터네트워크", "데이터베이스",
        "머신러닝", "인공지능", "소프트웨어공학", "컴퓨터구조",
        "미적분학", "선형대수", "통계학", "물리학", "화학",
        "경영학", "경제학", "마케팅", "회계", "법학",
        # 업무 관련
        "기획", "분석", "전략", "제안", "검토", "평가",
    ],
    "프레젠테이션": [
        # 발표 형식
        "발표자료", "세미나", "컨퍼런스", "시연", "워크숍",
        "프로젝트발표", "연구발표", "업무보고", "강의자료",
        # 내용 분야
        "기술발표", "서비스소개", "제품발표", "연구결과",
        "팀프로젝트발표", "졸업발표", "캡스톤디자인",
        "인턴발표", "실습발표", "교육자료",
    ],
    "스프레드시트": [
        # 재무·회계
        "예산", "정산", "매출", "재무", "가계부", "지출내역",
        # 통계·분석
        "통계", "집계", "데이터분석", "설문결과", "실험데이터",
        # 관리·일정
        "재고", "일정", "현황", "출석부", "성적표", "근태",
        "프로젝트관리", "업무현황",
    ],
    "코드": [
        # 기술 분야
        "웹개발", "앱개발", "백엔드", "프론트엔드", "풀스택",
        "머신러닝", "딥러닝", "인공지능", "데이터사이언스",
        "알고리즘", "자료구조", "운영체제", "데이터베이스",
        "네트워크", "보안", "암호화", "시스템프로그래밍",
        # 과제·실습
        "프로그래밍과제", "코딩실습", "알고리즘과제", "팀프로젝트",
        "오픈소스", "토이프로젝트", "포트폴리오",
    ],
    "데이터": [
        # 분석 유형
        "탐색적분석", "통계분석", "시각화", "전처리", "정제",
        # 데이터 출처
        "설문데이터", "실험데이터", "로그데이터", "크롤링데이터",
        "공공데이터", "수집데이터",
        # 처리 단계
        "파이프라인", "ETL", "분석결과", "모델결과",
    ],
...
def infer_tag(text: str, category: str, threshold: float = 0.35) -> Optional[str]:
    ...
    태그 추론은 문서 도입부(제목·첫 문단)가 정보 밀도가 높으므로 앞 300자만 사용.
    """
    ...
    try:
        model = _get_model()
        # 앞 300자: 제목·헤더 영역이 태그 정보가 가장 집중되어 있음
        text_emb = model.encode(text.strip()[:300])
...
def load_custom_categories(custom_categories: list[dict]) -> None:
    ...
    # 내장 태그 후보 캐시도 초기화 — TAG_CANDIDATES 변경 시 재계산 보장
    _tag_embeddings.clear()
...
def apply_feedback(text: str, correct_category: str) -> None:
    ...
    try:
        ...
        # 사용자가 명시적으로 수정한 피드백이므로 0.15로 더 빠르게 반영
        learning_rate = 0.15
